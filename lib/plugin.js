...
var DecisionProviderRegistry = class {
  ...
  register(provider, options = {}) {
    ...
    if (this.#pendingDefaultId === id && (options.enabled ?? true)) {
      this.#defaultId = id;
      this.#pendingDefaultId = void 0;
    } else if (this.#defaultId === void 0 && (options.enabled ?? true)) {
      this.#defaultId = id;
    }
    return () => {
  ...
  getDefaultId() {
    return this.#pendingDefaultId === void 0 ? this.#defaultId : void 0;
  }
  ...
  resolve(requestedId) {
    ...
    if (this.#pendingDefaultId !== void 0) {
...
var DecisionRouter = class {
  ...
  setDefaultProvider(id) {
    if (id === void 0) this.#registry.resetDefault();
...
function sanitizeUsage(value) {
  ...
  if (typeof metrics === "object" && metrics !== null && !Array.isArray(metrics)) {
    const kept = {};
    for (const [name2, raw] of Object.entries(metrics)) {
      if (typeof raw === "number" && Number.isFinite(raw)) kept[name2] = raw;
    }
...
var DecisionRuntime = class {
  ...
  async #drive(options, config, adapter, pending) {
    ...
    const phase = async (name2, limit, work) => {
      ...
      const phaseDetails = { phase: name2, ...name2 === "execute" ? { actionMayHaveExecuted: true } : {} };
      const onAbort = () => stop(new DecisionError("aborted", `Cancelled during ${name2}.`, { details: phaseDetails }));
      options.signal?.addEventListener("abort", onAbort, { once: true });
      const timer = setTimeout(() => stop(new DecisionError("budget_exhausted", `${name2} exceeded its ${ms}ms budget.`, { details: phaseDetails })), ms);
      ...
      try {
        ...
        if (error instanceof DecisionError) throw error;
        const fallback = name2 === "execute" ? "action_execution_failed" : name2 === "map action" ? "action_mapping_failed" : "internal";
...
// src/assembly.ts
function assembleDecisionCore(options) {
  ...
  for (const entry of options.providers) {
    providers.register(entry.provider, {
      ...
      ...entry.config === void 0 ? {} : { config: entry.config }
    });
  }
  const requested = options.defaultProvider;
  if (requested !== void 0) {
    const entry = providers.entry(requested);
    if (entry === void 0 && !options.deferMissingDefault) {
    ...
    else if (entry === void 0 && options.deferMissingDefault) providers.deferDefault(requested);
  }
  const active = providers.getDefaultId();
  const engine = new DecisionEngine({
    ...active === void 0 ? {} : { defaultProviderId: active },
...
async function aggregateDecisionHealth(options) {
  const providerHealth = await options.providers.health();
  const statuses = Object.values(providerHealth).map((entry) => entry.status);
  const providerStatus = statuses.length === 0 || statuses.every((entry) => entry === "unavailable") ? "unavailable" : statuses.every((entry) => entry === "ok") ? "ok" : "degraded";
  const status = providerStatus === "unavailable" ? providerStatus : options.providers.getPendingDefaultId() === void 0 ? providerStatus : "degraded";
  const defaultProvider = options.providers.getDefaultId();
  return {
    status,
    ...defaultProvider === void 0 ? {} : { defaultProvider },
    ...options.requestedDefault === void 0 || options.requestedDefault === defaultProvider ? {} : { requestedDefaultProvider: options.requestedDefault },
...
var IFRAME_HEADER_RE = /^---\s+iframe\b/;
...
var DEFAULT_LAYA_IDLE_TTL_MS = 10 * 6e4;
var LayaConfigSchema = z.object({
...
var Config = z2.object({
  ...
  runtime: z2.object({
    maxSteps: z2.number().default(10).description(
      "Hard step limit for one bounded loop. The run escalates instead of exceeding it."
    ),
    maxDurationMs: z2.number().default(12e4).description(
      "Hard wall-clock limit for one bounded loop, in milliseconds."
    ),
    confidenceThreshold: z2.number().default(0.55).description(
    ...
    noProgressLimit: z2.number().default(3).description(
      "Stop a loop after this many consecutive steps in which the environment state did not change."
    ),
    repeatedDecisionLimit: z2.number().default(3).description(
      "Stop a loop after the same candidate is chosen this many times in a row."
    ),
    observeTimeoutMs: z2.number().default(9e4).description(
      "Budget for one observation, and the default provider budget for a single decision, in milliseconds."
    ),
    executeTimeoutMs: z2.number().default(9e4).description(
      "Budget for executing one environment action, in milliseconds."
    ),
    stepDelayMs: z2.number().default(0).description(
      "Pause between loop steps, in milliseconds, so a page or app can settle."
    ),
    stateFingerprintChars: z2.number().default(2e3).description(
      'How many characters of environment state are compared to detect "no progress".'
    ),
    singleCandidateSteps: z2.union([z2.const("ask"), z2.const("execute")]).default("ask").description(
      'What to do when a step offers exactly one candidate: ask the provider (default), or take it directly. Stage scopes that narrow to a single control need "execute" \u2014 there is nothing to decide, and a small local head fails on it.'
    )
  }).description("Budgets and stop conditions shared by every environment."),
  browser: z2.object({
    includeNonSemantic: z2.boolean().default(false).description("Opt in to inferred clickable elements. Requires browser workspace v0.1.10 or newer."),
    candidateSelector: z2.string().description("CSS selector limiting inventory candidates before size caps. Prefer task-local overrides for site-specific selectors."),
    enabled: z2.boolean().default(true).description("Whether the browser environment is available to the decision layer."),
    environmentId: z2.string().description('Environment id to register it under. Defaults to "browser".'),
    maxCandidates: z2.number().description("Maximum number of page controls offered to the decider. Defaults to 12."),
    maxStateChars: z2.number().description("How much page text is placed into the decision state. Defaults to 6000."),
    candidates: z2.array(z2.any()).description(
  ...
  computer: z2.object({
    enabled: z2.boolean().default(true).description("Whether the desktop (accessibility) environment is available."),
    environmentId: z2.string().description('Environment id to register it under. Defaults to "computer".'),
    app: z2.string().description(
    ...
    maxCandidates: z2.number().description("Maximum number of accessibility elements offered to the decider. Defaults to 12."),
    maxStateChars: z2.number().description("How much accessibility-tree text is placed into the decision state. Defaults to 8000."),
    maxTreeNodes: z2.number().description("Maximum accessibility nodes captured per observation. Defaults to 1200."),
    captureTimeoutMs: z2.number().description(
  ...
  telemetryLimit: z2.number().default(200).description(
...
function createDecisionEngineComposition(options) {
  ...
  if (config.browser?.enabled ?? true) {
    ...
    const hasBrowserPatch = browserCandidates !== void 0 && browserCandidates.length > 0;
  ...
  const includeLaya = layaConfig.enabled !== false && (options.providers === void 0 || config.providers?.laya !== void 0);
  const providerSpecs = [
    ...includeLaya ? [createLayaProviderSpec(layaConfig)] : [],
    ...options.providers ?? options.extraProviders ?? []
  ];
  const disabledDefault = config.defaultProvider !== void 0 && config.providers?.[config.defaultProvider]?.enabled === false;
  const { providers, engine, runtime } = assembleDecisionCore({
    providers: providerSpecs,
    ...config.defaultProvider === void 0 || disabledDefault ? {} : { defaultProvider: config.defaultProvider },
    ...options.deferMissingDefault === void 0 ? {} : { deferMissingDefault: options.deferMissingDefault },
    ...config.runtime === void 0 ? {} : { runtime: config.runtime },
  ...
  const setDefaultProvider = (requested) => {
    const isDisabled = requested !== void 0 && config.providers?.[requested]?.enabled === false;
    if (requested !== void 0 && !providers.has(requested) && !isDisabled && !options.deferMissingDefault) {
      throw new DecisionError("provider_unknown", `defaultProvider "${requested}" is not registered`);
    }
    if (requested !== void 0 && providers.entry(requested)?.enabled) {
      engine.reconfigure({ defaultProviderId: requested });
    } else if (requested !== void 0 && !providers.has(requested) && !isDisabled && options.deferMissingDefault) {
      providers.deferDefault(requested);
    } else {
      engine.router.setDefaultProvider(void 0);
    }
  ...
  const service = {
    ...
    health: () => aggregateDecisionHealth({
      providers,
      environments,
      records,
      ...requestedDefault === void 0 ? {} : { requestedDefault }
    }),
    telemetry: () => records,
    dispose: async () => {
...
// src/tools/browser-options.ts
var BROWSER_OPTIONS_PARAMETER = {
  type: "object",
  ...
  properties: {
    includeNonSemantic: { type: "boolean", description: "Include inferred clickable div/span controls. Requires browser workspace v0.1.10+." },
    candidateSelector: { type: "string", description: "CSS selector filtering controls before size caps. Include the task controls and required navigation; an empty match never falls back to the whole page." },
    maxCandidates: { type: "integer", description: "Candidate cap, 1\u201364. Defaults to the registered adapter setting (12)." }
  }
};
function taskEnvironment(service, id, browser) {
  if (browser === void 0) return id;
  if (id === void 0 || browser === null || typeof browser !== "object" || Array.isArray(browser)) throw new DecisionError("invalid_request", "Browser settings require a browser environment and an options object.");
...
function preflightDecideInput(input, service) {
  ...
  if (!hasEnvironment && input.browser !== void 0) return "Browser settings require a browser environment.";
  if (!hasEnvironment) {
...
function projectOutcome(outcome2) {
  ...
  return {
    ...
    ...outcome2.decision === void 0 ? {} : {
      ...
      ...outcome2.decision.usage?.inputTokens === void 0 ? {} : { inputTokens: outcome2.decision.usage.inputTokens },
...
function renderDecideOutput(output) {
  ...
  if (output.inputTokens !== void 0) lines.push(`Input tokens: ${output.inputTokens}`);
...
async function executeDecide(input, context, signal) {
  ...
  if (input.browser !== void 0) throw new DecisionError("invalid_request", "Browser settings require a browser environment.");
  if (input.state === void 0) {
...
// src/tools/decision-decide.ts
function defineDecideTool(context, scope) {
  const { service } = context;
  return defineTool({
    ...
    execute: async (args, exec) => {
      ...
      return scope === void 0 ? work() : scope(exec, work);
    }
  });
}

// src/tools/decision-run.ts
...
// src/tools/decision-run.ts
async function executeRunTask(input, service, signal) {
  ...
  if (input.environment === void 0 === (input.endpoint === void 0)) throw new DecisionError("invalid_request", "Pass exactly one of environment or endpoint.");
  if (input.maxDurationMs !== void 0 && input.maxDurationMs > 18e5) throw new DecisionError("invalid_request", "DSH tasks may run for at most 30 minutes per call.");
  if (input.endpoint !== void 0 && input.browser !== void 0) throw new DecisionError("invalid_request", "Browser settings cannot be used with an HTTP endpoint.");
  const environment = input.endpoint === void 0 ? taskEnvironment(service, input.environment, input.browser) : new HttpEnvironmentAdapter({ endpoint: input.endpoint });
  return service.runTask({
    environment,
    objective: { description: input.objective, ...input.completion === void 0 ? {} : { completion: input.completion } },
    ...
    config: {
      ...
      ...input.maxDurationMs === void 0 ? {} : { maxDurationMs: input.maxDurationMs }
    }
  });
}
function defineRunTool(service, scope) {
  return defineTool2({
    name: "decision_run",
    description: "Hand over an entire game, website, or desktop task to the decision executor. It independently observes, decides and executes until completion or escalation, then returns the final result and score. Use endpoint for a dsh-environment/v1 HTTP API, or environment for a registered adapter. Do not fetch state, forward questions, click next, or call this once per move: the executor owns all intermediate steps. Pass the main agent's ordered plan once; the small model selects actions within each stage, and observed completion advances it. Browser/desktop tasks need a plan with completion rules, a completion rule, or an adapter that reports completion.",
    timeoutMs: 186e4,
    parameters: {
      ...
      plan: {
        type: "array",
        description: "Ordered execution plan from the main agent. No main-agent intervention between stages.",
        items: {
          type: "object",
          additionalProperties: false,
          properties: {
            ...
            scope: {
              ...
              description: "What the driver may do while this stage is active, interpreted by the environment adapter. For the browser environment: candidateSelector, includeNonSemantic, maxCandidates. Narrowing a stage removes the wrong choices instead of asking the model to ignore them."
            },
            completion: {
              type: "object",
              required: true,
    ...
    execute: async (args, execution) => {
      const work = async () => {
        const outcome2 = await executeRunTask(args, service, execution.signal);
        const value = {
          taskId: outcome2.taskId,
          status: outcome2.status,
          environment: outcome2.environment,
          steps: outcome2.steps,
          durationMs: outcome2.durationMs,
          ...outcome2.result === void 0 ? {} : { result: outcome2.result },
          ...typeof outcome2.finalState === "string" || typeof outcome2.finalState === "object" && outcome2.finalState !== null && !Array.isArray(outcome2.finalState) ? { finalState: outcome2.finalState } : {},
          ...outcome2.escalation === void 0 ? {} : { reason: outcome2.escalation.reason, guidance: outcome2.escalation.guidance },
          ...outcome2.stopReason === void 0 ? {} : { stopReason: outcome2.stopReason },
          ...outcome2.escalation?.details?.actionMayHaveExecuted === true ? { actionMayHaveExecuted: true } : {},
          ...outcome2.completedPlanSteps === void 0 ? {} : { completedPlanSteps: outcome2.completedPlanSteps },
          ...outcome2.activePlanStep === void 0 ? {} : { activePlanStep: outcome2.activePlanStep }
        };
        return JSON.parse(JSON.stringify(value));
      };
      return scope === void 0 ? work() : scope(execution, work);
    }
  });
}
...
Each stage may also carry a \`scope\` object: what the driver is allowed to do while that stage is
active. It is handed to the environment adapter, which owns the keys \u2014 for the browser that is
\`includeNonSemantic\`, \`candidateSelector\` and \`maxCandidates\`. A stage without \`scope\` keeps the
call-level configuration, and adapters without \`withConfig\` ignore the field. Narrowing a stage is how
...
// src/provider-catalog-route.ts
var PROVIDER_CATALOG_ROUTE = "/plugins/dsh-decision-engine/providers";
function serveProviderCatalog(req, res, enabledIds) {
...
var HostToolDispatcher = class {
  ...
  #execution;
  #callCounter = 0;
  constructor(ctx, execution = () => void 0) {
    this.#ctx = ctx;
  ...
  async call(request) {
    ...
    try {
      const result = await tools.execute({
        ...
        ...agent === void 0 ? {} : { agent },
        ...execution === void 0 ? {} : { parent: execution.token, rootCallId: execution.rootCallId },
...
var SETTINGS_NAMESPACE = "decision-engine";
function apply(ctx, config = {}) {
  if (config.enabled === false) return;
  const execution = new AsyncLocalStorage();
  const scope = (caller, work) => execution.run(caller, work);
  const dispatcher = new HostToolDispatcher(ctx, () => execution.getStore());
  const gate = () => ctx.get(TOOL_LAZY_GATE_SERVICE);
  const settings = ctx.get("settings");
  const settingsScope = settings?.register(SETTINGS_NAMESPACE, Config, {
    base: config,
    applies: "live",
    validate: (value) => {
      if (value.defaultProvider !== void 0 && value.defaultProvider.trim() === "") {
  ...
  const initialConfig = settingsScope?.get() ?? config;
  ...
  if (settingsScope !== void 0) {
    const applySettings = () => {
      ...
      try {
        const engineConfig = {};
        if (resolved.runtime?.confidenceThreshold !== void 0) engineConfig.confidenceThreshold = resolved.runtime.confidenceThreshold;
        if (resolved.runtime?.observeTimeoutMs !== void 0) engineConfig.timeoutMs = resolved.runtime.observeTimeoutMs;
        composition.setDefaultProvider(resolved.defaultProvider);
        composition.service.engine.reconfigure(engineConfig);
        if (resolved.runtime !== void 0) composition.service.runtime.reconfigure(resolved.runtime);
      } catch (error) {
  ...
  ctx.systemPrompt.section({
    name: "tool:decision",
    order: 106,
    text: 'Plan the task, then call `decision_run` once with its objective, ordered plan with completion conditions, and environment or API endpoint. The executor owns observation, decisions, actions and completion checks; do not relay state or perform intermediate actions yourself. The small model executes within the supplied plan. Verify the final result after it returns; do not intervene between steps. It returns the final score/result, plan progress or an escalation. For an individual choice, the decision layer answers with a finite candidate set: call `decision_decide` with an objective, the environment state (or an environment id), and the candidates. It decides by default; pass execute: true to run one mapped action, or execute: "loop" for a bounded loop. When the environment cannot express the task as structured state it returns status "needs_escalation" \u2014 take the step over yourself instead of retrying.'
  });
}
export {
