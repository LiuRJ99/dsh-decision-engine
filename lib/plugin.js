...
function assembleDecisionCore(options) {
  ...
  if (requested !== void 0) {
    ...
    if (entry === void 0 && !(requested === "laya" && options.laya === false)) {
...
var BrowserEnvironmentAdapter = class _BrowserEnvironmentAdapter {
  ...
  buildDecisionRequest(observation, objective) {
    ...
    const state = {
      ...
      title: snapshot.title ?? "",
      status: snapshot.status ?? "",
      pageTextUntrusted: truncate2(snapshot.main, this.#config.maxStateChars),
    ...
    return {
      objective: truncate2(objective.description, this.#config.maxObjectiveChars),
      state,
  ...
  buildDecisionRequest(observation, objective) {
    ...
    return {
      ...
      state: {
        ...
        treeTextUntrusted: truncate3(state.text, this.#config.maxStateChars)
      },
      candidates: candidates.map((candidate) => ({
        id: candidate.id,
        description: candidate.description,
...
function createDecisionEngineComposition(options) {
  ...
  const { providers, engine, runtime } = assembleDecisionCore({
    ...
    ...options.extraProviders === void 0 ? {} : { extraProviders: options.extraProviders },
  ...
  const setDefaultProvider = (requested) => {
    if (requested !== void 0 && !providers.has(requested) && !(requested === "laya" && layaConfig.enabled === false)) {
      throw new DecisionError("provider_unknown", `defaultProvider "${requested}" is not registered`);
    }
    const active = requested !== void 0 && providers.entry(requested)?.enabled ? requested : providers.enabledIds()[0];
    if (active !== void 0) engine.reconfigure({ defaultProviderId: active });
...
function apply(ctx, config = {}) {
  ...
  const settingsScope = settings?.register(SETTINGS_NAMESPACE, Config, {
    ...
    applies: "live",
    validate: (value) => {
      if (activeComposition !== void 0 && value.defaultProvider !== void 0 && !activeComposition.providers.has(value.defaultProvider) && !(value.defaultProvider === "laya" && value.providers?.laya?.enabled === false)) {
