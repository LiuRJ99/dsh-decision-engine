...
function apply(ctx, config = {}) {
  ...
  ctx.inject(["settings"], (settingsCtx) => {
    ...
    const scope2 = settings.register(SETTINGS_NAMESPACE, Config, {
      base: config,
      applies: "live"
    });
    ctx.effect(() => () => {
    ...
    const applySettings = () => {
      ...
      try {
        const engineConfig = {};
        if (resolved.defaultProvider !== void 0) engineConfig.defaultProviderId = resolved.defaultProvider;
    ...
    applySettings();
    ctx.effect(() => scope2.watch(() => applySettings()), "decision-engine settings watch");
  });
