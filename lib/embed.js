...
function createDecisionLayer(options = {}) {
  ...
  const layaOption = options.laya ?? options.providers === void 0;
  ...
  const { providers, engine, runtime } = assembleDecisionCore({
    ...
    ...options.defaultProvider === void 0 ? {} : { defaultProvider: options.defaultProvider },
  ...
  return {
    ...
    health: () => aggregateDecisionHealth({
      ...
      environments,
      records,
      ...options.defaultProvider === void 0 ? {} : { requestedDefault: options.defaultProvider }
    }),
    dispose: async () => {
