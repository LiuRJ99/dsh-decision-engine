...
export const GATE_SKILL_NAMES: Record<'browser' | 'computer', string> = {
  browser: 'browser',
...
export function isToolLazyGateSurface(value: unknown): value is ToolLazyGateSurface {
  return typeof value === 'object'
    && value !== null
    && typeof (value as { isUnlocked?: unknown }).isUnlocked === 'function'
}
 ...
 */
export function queryCapabilityUnlocked(
  gate: unknown,
  agent: unknown,
  capability: 'browser' | 'computer',
): boolean | undefined {
  if (!isToolLazyGateSurface(gate)) return undefined
